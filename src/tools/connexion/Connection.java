...
public class Connection extends Thread {
	...
	/**
	 * objet de lien avec une autre classe qui implémente AsyncResponse pour
	 * transférer les réponses
	 */
	private AsyncResponse delegate;

	/**
	 * Constructeur : crée une connexion à partir d'un socket (contenant les
	 * spécificités de l'ordinateur distant)
	 * 
	...
	public Connection(Socket socket, AsyncResponse delegate) {
		...
		this.start();
		// envoi de l'instance de connexion vers la classe qui implémente AsyncResponse
	...
	public synchronized void envoi(Object unObjet) {
		// l'envoi ne peut se faire que si un objet delegate existe (pour récupérer la
		// réponse)
		if (delegate != null) {
			try {
				this.out.reset();
	...
	public void run() {
		...
		while (inOk) {
			try {
				...
				// envoi de l'information reçue vers la classe qui implémente AsyncResponse pour
				...
				// envoi de l'information de déconnexion vers la classe qui implémente
				// AsyncResponse pour récupérer la réponse
