...
import tools.son.ecouteurs.EcouteurSon;
import tools.son.exceptions.SonErreurDiverse;
import tools.son.exceptions.SonErreurLecture;
import tools.son.exceptions.SonException;
import tools.son.exceptions.SonIntrouvableException;
import tools.son.exceptions.SonTypeException;

/**
 * Représente un son. <br>
 * Les sons les mieux suppoter par Java sont MIDI, AU et certains WAV. <br>
 * Pour savoir si un son est accepté, le seul moyen est d'essayer. <br>
 * Si le son ne sera plus utilisé, il faut le détruire par la Méthode fermer,
 * <br>
 * afin de libèrer la mémoire et le flux où se trouve le son. <br>
...
public class Sound implements Serializable, Runnable {
	...
	// fermerALaFin : indique si le son doit être détruit une fois la dernière
	// boucle de son exécutée
	private boolean pause, fermerALaFin;
	// Ecouteurs des évènement sons
	...
	private void initialise(File fichier) throws SonException {
		try {
			...
			// On ne peut pas ouvrir directement des format ALAW/ULA, il faut les convertir
			// en PCM
			if ((this.format.getEncoding() == AudioFormat.Encoding.ULAW)
				...
				AudioFormat tmp = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, this.format.getSampleRate(),
			...
			// On crée une information avec le format du flux et en caculant la logueneur
			// totale du son
			DataLine.Info info = new DataLine.Info(Clip.class, this.lecteurAudio.getFormat(),
	...
	/**
	 * Action du son, ne jamais appelé cette Méthode directement, elle est public
	 * pour respecter l'implémentation de Runnable
	 */
	public void run() {
	 ...
	 * 
	 * @param fermer <b>true</b> pour indiqué que l'on désire que le son soit
	 *               détruit après la dernière fois qu'il joue
	 */
	...
	/**
	 * Ajout un écouteur d'évènement son
	 * 
	...
	/**
	 * Retire un écouteur d'évènement son
	 * 
	...
	// Indique à tout les écouteurs d'évènements son, que le son est terminé
	...
	// Indique à tout les écouteurs d'évènements son, que le son a avancé
